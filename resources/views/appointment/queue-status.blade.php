...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function getStatusBadge(status) {
            ...
            return badges[status] || `<span class="px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-800">${status}</span>`;
        }

        async function fetchQueueData(date) {
            if (!date) return;
            ...
            try {
                const response = await fetch(`/appointments/queue-status-data?date=${date}`);
                ...
                if (data.appointments && data.appointments.length > 0) {
                    data.appointments.forEach(app => {
                        ...
                        if (app.status === 'started') {
                            tr.className = 'bg-green-50/60 font-medium scale-[1.01] shadow-sm rounded-lg';
                        } else {
        ...
        dateInput.addEventListener('change', (e) => {
            fetchQueueData(e.target.value);
        });

        // Initial fetch
        if (dateInput.value) {
            fetchQueueData(dateInput.value);
        }
            ...
            try {
                ...
                if (dateInput.value === todayStr && currentQueueNum) {
                    ...
                    if (currentDisplayed !== currentQueueNum && currentQueueNum > 0) {
                        // Queue changed, fetch new data for today
                        fetchQueueData(todayStr); 
                    }
                }
            } catch(e) {
