    ...
    <header class="bg-white/80 backdrop-blur-md shadow-sm border-b border-slate-200 sticky top-0 z-40 transition-all">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <a href="{{ route('staff.dashboard') }}" class="flex items-center space-x-3 group hover:opacity-90 transition-opacity">
                    <div class="bg-gradient-to-br from-emerald-500 to-emerald-600 p-2 rounded-xl shadow-sm shadow-emerald-200 group-hover:scale-105 transition-transform">
        ...
        <div class="max-w-7xl mx-auto space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                ...
                <div class="px-6 py-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <div class="text-sm text-slate-500 mt-1 flex items-center flex-wrap gap-2">
                            Showing records for:
                            <span class="font-semibold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-md border border-emerald-100">
                                @if ($date === 'all')
                                    All time
                                @else
                    ...
                    @if ($patientId)
                        <a href="{{ route('staff.medical-records', ['date' => $date ?? now()->toDateString(), 'search' => $search ?? request('search')]) }}"
                            class="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-600 hover:text-emerald-600 bg-white border border-slate-200 hover:border-emerald-200 hover:bg-emerald-50 px-4 py-2 rounded-xl transition-all shadow-sm">
                ...
                <div class="px-6 py-4 bg-slate-50 border-b border-slate-100">
                    <form method="GET" action="{{ route('staff.medical-records') }}" id="filterForm"
                        ...
                        <div class="flex-1">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="search" value="{{ $search ?? request('search') }}"
                                    ...
                                    class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all shadow-sm placeholder:text-slate-400 text-slate-700 font-medium">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-4">
                                <input type="date" name="date" id="dateInput" value="{{ $date !== 'all' ? ($date ?? now()->toDateString()) : now()->toDateString() }}"
                                    class="px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all shadow-sm text-slate-700 font-medium {{ $date === 'all' ? 'opacity-50 bg-slate-100 cursor-not-allowed text-slate-400' : '' }}" 
                                    {{ $date === 'all' ? 'disabled' : '' }}>
                                ...
                                <label for="showAll" class="flex items-center gap-2 cursor-pointer select-none group">
                                    <div class="relative flex items-center">
                                        <input type="checkbox" id="showAll" {{ $date === 'all' ? 'checked' : '' }}
                                            class="w-5 h-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 focus:ring-offset-0 cursor-pointer transition-all shadow-sm">
                                    </div>
                            ...
                            @forelse($records as $record)
                                <tr class="hover:bg-slate-50/80 transition-colors group">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <a href="{{ route('staff.medical-records', ['patient_id' => $record->patient_id, 'date' => $date ?? now()->toDateString()]) }}"
                                            class="font-bold text-emerald-600 hover:text-emerald-800 hover:underline transition-colors">
    ...
    <dialog id="viewRecordModal" class="p-0 bg-transparent backdrop:bg-slate-900/50 open:animate-in open:fade-in open:zoom-in-95 rounded-2xl shadow-xl w-full max-w-2xl m-auto">
        <div class="bg-white rounded-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                ...
                <button type="button" onclick="printPrescription()" class="inline-flex items-center gap-2 bg-emerald-600 text-white hover:bg-emerald-700 px-5 py-2 rounded-xl text-sm font-semibold transition-all shadow-sm focus:ring-2 focus:ring-emerald-500/20 outline-none">
        ...
        function viewRecord(btn) {
            ...
            if (prescriptions.length === 0) {
                ...
                prescriptions.forEach(rx => {
                    ...
                    row.innerHTML = `
                        <div class="font-bold text-emerald-700 mb-2 flex items-center gap-2">
