...
<head>
    ...
    <style>
        ...
        @keyframes pulse-glow {
            0%, 100% { filter: drop-shadow(0 0 15px rgba(74, 222, 128, 0.4)); }
...
<body class="text-white min-h-screen flex flex-col relative">
    ...
    <div class="orb-1"></div>
    <div class="orb-2"></div>
    <div class="absolute inset-0 bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAiIGhlaWdodD0iMjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGNpcmNsZSBjeD0iMSIgY3k9IjEiIHI9IjEiIGZpbGw9InJnYmEoMjU1LDI1NSwyNTUsMC4wNSkiLz48L3N2Zz4=')] opacity-30 z-0"></div>

    <!-- Header -->
    <header class="relative z-10 w-full p-8 flex justify-between items-center border-b border-white/10 bg-black/20 backdrop-blur-md">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-full bg-gradient-to-tr from-green-400 to-blue-500 flex items-center justify-center shadow-lg shadow-green-500/30">
            ...
            <div>
                <h2 class="text-3xl font-bold tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-blue-400">RURAL HEALTH UNIT</h2>
                <p class="text-slate-400 text-sm font-medium tracking-widest uppercase mt-1">Outpatient Department Queue</p>
            </div>
        </div>
    ...
    <main class="relative z-10 flex-1 flex flex-col items-center justify-center p-8">
        ...
        <div class="w-full max-w-5xl relative">
            <!-- Glassmorphic Card -->
            <div class="absolute inset-0 bg-gradient-to-b from-white/5 to-transparent rounded-[3rem] blur-xl opacity-50"></div>
            
            <div class="relative bg-slate-900/60 backdrop-blur-2xl border border-white/10 rounded-[3rem] p-20 shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col items-center justify-center overflow-hidden">
                ...
                <div class="relative z-10 text-center flex flex-col items-center w-full">
                    <div class="inline-flex items-center gap-4 px-8 py-3 rounded-full bg-slate-800/80 border border-slate-700/50 mb-12 shadow-inner">
                    ...
                    <div id="queueContainer" class="number-transition transform scale-100 transition-all duration-500 ease-out flex justify-center w-full min-h-[250px] items-center">
                        <div id="queueNumber" class="text-[12rem] md:text-[15rem] leading-none font-black text-slate-600 drop-shadow-2xl">
                            WAITING...
                        </div>
                    </div>
                    
                    <div class="h-10 mt-8 mb-4">
                        <p id="queueStatus" class="text-3xl font-medium text-slate-400 tracking-wide opacity-0 transition-opacity duration-500">
                ...
                source.onmessage = function(event) {
                    ...
                    if (currentDisplayNumber !== qNumber) {
                        ...
                        if (currentDisplayNumber !== null && qNumber !== 0) {
                            try { playChime(); } catch(e) { console.log('Audio blocked by browser policy'); }
                        }
                        ...
                        setTimeout(() => {
                            if (qNumber === 0) {
                                ...
                                queueNumberElement.className = "text-[7rem] md:text-[9rem] leading-none font-black text-slate-700 drop-shadow-lg";
                                ...
                                queueNumberElement.className = "text-[12rem] md:text-[16rem] leading-[0.9] font-black text-transparent bg-clip-text bg-gradient-to-b from-green-300 to-green-600 pulse-glow drop-shadow-2xl";
